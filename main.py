...
def system(t: float, Y: np.ndarray, m_value: float, p_value: float, derivatives: np.ndarray) -> None:
    ...
    # Основное уравнение
    derivatives[0] = (pressure_term - F(Y[1], Y[0])) * angle_value / mass_term - g(Y[1])  # производная z
    derivatives[1] = Y[0]  # производная y

    # Изохронная производная по m0
    derivatives[2] = -(pressure_term - F(Y[1], Y[0])) * angle_value / mass_term ** 2  # производная zm
    derivatives[3] = Y[2]
...
def gauss_newton(initial_m: float, initial_p: float, tol: float = 1e-6) -> list:
    ...
    while True:
        ...
        for i in data[:, 0]:
            y_model_fix.append(solution[1][int(i * 10)])
            rm_model_fix.append(solution[3][int(i * 10)])
            rp_model_fix.append(solution[5][int(i * 10)])

        residuals[:] = y_model_fix - data[:, 2]
...
def main_init(m = 313000, p = 5042983):
    final_solution = np.zeros((6, 1180))  # Здесь 1180 - максимальное количество шагов
    t_values = np.arange(0, 118, 0.1)  # Создаем массив временных значений от 0 до 118 с шагом 0.1
    num_steps = len(t_values) - 1  # Общее количество шагов
    dt = 0.1
    half_dt = dt / 2
    k = np.zeros((6, 4))
